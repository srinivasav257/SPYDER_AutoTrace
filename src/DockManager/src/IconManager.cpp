...
void drawDashboard(QPainter& painter, const QRectF& rect, const QColor& color)
{
    ...
    const qreal s = 4.2;
    const qreal gap = 1.8;
    const qreal startX = rect.center().x() - (s + gap / 2.0);
    const qreal startY = rect.center().y() - (s + gap / 2.0);

    painter.drawRoundedRect(QRectF(startX, startY, s, s), 0.8, 0.8);
    painter.drawRoundedRect(QRectF(startX + s + gap, startY, s, s), 0.8, 0.8);
    painter.drawRoundedRect(QRectF(startX, startY + s + gap, s, s), 0.8, 0.8);
    painter.drawRoundedRect(QRectF(startX + s + gap, startY + s + gap, s, s), 0.8, 0.8);
}
...
QIcon icon(Id id, const QWidget* context)
{
    ...
    switch (id) {
    case Id::Import:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawTrayArrow(p, r, c); });
    case Id::Save:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawSaveDisk(p, r, c); });
    case Id::AddGroup:
    ...
    case Id::Remove:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawRemove(p, r, c); });
    case Id::ExpandAll:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawChevronPair(p, r, c, true); });
    case Id::CollapseAll:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawChevronPair(p, r, c, false); });
    case Id::PanelExplorer:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawExplorer(p, r, c); });
    case Id::PanelProgress:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawProgress(p, r, c); });
    case Id::GroupItem:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawFolder(p, r, c); });
    case Id::FeatureItem:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawDiamond(p, r, c); });
    case Id::TestItem:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawDocument(p, r, c); });
    case Id::Run:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawPlay(p, r, c); });
    case Id::ActivityDashboard:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawDashboard(p, r, c); });
    case Id::ActivityCanalyzer:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawCanalyzer(p, r, c); });
    case Id::ActivitySettings:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawSettings(p, r, c); });
    case Id::ActivityProfile:
        return buildIcon(context, [](QPainter& p, const QRectF& r, const QColor& c) { drawProfile(p, r, c); });
    }
