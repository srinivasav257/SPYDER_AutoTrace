...
version_info = {"SW_Build_Version": "Not able to Read",
                ...
                "Boot_SoftwareVersion": "Not able to Read",
                "Application_SoftwareVersion": "Not able to Read"}
...
def Read_System_Info(diag):
    ...
    commands = {
        ...
        "AP_Boot_Up_status_cmd": "6D643E 00 05 00 00 00",
        "SilverBox_HW_Version": "6D643E 78 01 00 00 01 00",
        "Boot_SoftwareVersion": "6D643E 64 02 00 00 03 00 81 00",
        "Application_SoftwareVersion": "6D643E 64 02 00 00 03 00 83 00",
    }
    ...
    for name, command in commands.items():
        ...
        try:
            if name == "SW_Build_Version":
                response_string = diag.serial_write_data_and_await_for_response(command, pattern="Project Name/Version")
                ...
                if not response or response.get("Status") != "01":
                    ...
                    continue
                if name == "SilverBox_HW_Version":
                    version_info[name] = hex_to_ascii_from_n(response.get("Databytes", ""), 1)

                elif name == "Boot_SoftwareVersion":
                    version_info[name] = hex_to_ascii_from_n(response.get("Databytes", ""), 2)

                elif name == "Application_SoftwareVersion":
                    version_info[name] = hex_to_ascii_from_n(response.get("Databytes", ""), 2)
