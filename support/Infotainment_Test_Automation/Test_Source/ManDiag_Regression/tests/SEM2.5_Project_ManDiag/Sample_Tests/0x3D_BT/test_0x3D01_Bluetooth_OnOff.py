# ------------------------------------------------------------------------------------------------------------------
# File Name:    test_0x3D01_Bluetooth_OnOff.py
# Author:       Srinivasa V
# Created:      Aug 22 2025
# Description:  This File contains Test case scenarios for validating 0x3D01 Bluetooth ON/OFF feature.
# ------------------------------------------------------------------------------------------------------------------
...
#@pytest.mark.order(7)
def test1_0x3D01_Bluetooth_OnOff(ManDiag_component):
    """ Bluetooth OFF test """
    # Step 0: Update the Test variable and Test Procedure
    test_description = """
                Test Case: Validate Bluettoth OFF
                Steps:
                1. Pre-condition: The Unit should be powered ON and VIP must be fully BootUp and Running.
                2. Actual Test: Send SET power Enable/Disable command to turn Bluetooth OFF.          
                3. Post-condition: Send GET command to verify the OFF status.

                Expected Result:
                - The response should indicate that Bluetooth is powered OFF.
                """
    app_logger.debug(test_description)
    app_logger.info("Starting test: Bluetooth OFF test feature Validation")
    SetCmd = "6D643E 3D 01 01 00 01 00"
    GetCmd = "6D643E 3D 01 00 00 00"

    # Step 1: Pre_Condition Test

    # Step 2: Actual Test Case
    app_logger.info("Executing actual test: Turn OFF BT ")
    ...
    app_logger.info("Executing Post Condition test: get BT status ")
...
# @pytest.mark.skip(reason="Enable this line for Skipping this test case")
#@pytest.mark.order(8)
def test2_0x3D01_Bluetooth_OnOff(ManDiag_component):
    """ Bluetooth ON test """
    # Step 0: Update the Test variable and Test Procedure
    test_description = """
                Test Case: Validate Bluetooth ON
                Steps:
                1. Pre-condition: The Unit should be powered ON and VIP must be fully BootUp and Running.
                2. Actual Test: Send SET power Enable/Disable command to turn Bluetooth ON.          
                3. Post-condition: Send GET command to verify the ON status.

                Expected Result:
                - The response should indicate that Bluetooth is powered ON.
                """
    app_logger.debug(test_description)
    SetCmd = "6D643E 3D 01 01 00 01 01"
    GetCmd = "6D643E 3D 01 00 00 00"

    # Step 1: Pre_Condition Test

    # Step 2: Actual Test Case
    app_logger.info("Executing actual test: Turn ON BT ")
    assert ManDiag_component.send_request_command_and_validate_response(SetCmd, expected_status="AA", expected_data_length="00",  retries=2)
    time.sleep(3)
