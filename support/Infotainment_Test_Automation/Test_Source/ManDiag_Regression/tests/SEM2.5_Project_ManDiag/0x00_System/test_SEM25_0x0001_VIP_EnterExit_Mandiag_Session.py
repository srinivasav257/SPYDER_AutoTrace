...
#
# This file is part of the Manufacture Diagnostics Component feature test Automation.
#
...
@pytest.mark.order(1)
def test1_0x0001_EnterManDiag_Session(ManDiag_component):
    ...
    test_description = """
            ...
            3. Post-condition: Send SET Enter ManDiag session cmd and validate the response .
    ...
    assert ManDiag_component.send_request_command_and_validate_response(SetCommand, expected_data_length="01", expected_data="01")
...
def test2_0x0001_ExitManDiag_Session(ManDiag_component):
    ...
    assert ManDiag_component.send_request_command_and_validate_response(SetCommand, expected_data_length="01", expected_data="00")

    # Step 3: Post Condition
    app_logger.info("Executing Post Condition test: Send GET Exit ManDiag session")
    assert ManDiag_component.send_request_command_and_validate_response(GetCommand, expected_status="00")
...
# @pytest.mark.skip(reason="disabling just for debugging purpose")
@pytest.mark.order(3)
def test3_0x0001_Send_AnyCmd_without_ManDiag_Session_Opening(ManDiag_component):
    """ IOC ManDiag : Exit ManDiag Session Operation """
    test_description = """
    ...
    assert ManDiag_component.send_request_command_and_validate_response(GetWatchDogStatus, expected_data_length="00", expected_status="00")
...
# ************************************************************* TEST CASES #04 *************************************************
#@pytest.mark.skip(reason="disabling this test because this condition is not handled in code")
@pytest.mark.order(4)
def test4_0x0001_invalid_operation_for_man_diag_session(ManDiag_component):
    ...
    SetCommand = "6D643E 00 01 01 00 01 01"
    SetCommand1 = "6D643E 00 01 01 00 01 02"
    ...
    assert ManDiag_component.send_request_command_and_validate_response(SetCommand, expected_data_length="01", expected_data="01")
    app_logger.info("Executing actual test: Send SET Exit ManDiag session")
    result = ManDiag_component.send_request_command_and_validate_response(SetCommand1, expected_data_length="00", expected_status="F6")
    if not result:
...
# @pytest.mark.skip(reason="disabling just for debugging purpose")
@pytest.mark.order(5)
def test5_0x0001_invalid_Data_length_for_man_diag_session(ManDiag_component):
    """ IOC ManDiag : Exit ManDiag Session Operation """
    test_description = """
            Test Case: Validate Exit ManDiag Session operation
            Steps:
            1. Pre-condition: The Unit should be powered ON and VIP must be fully BootUp and Running.
            2. Actual Test: Send SET invalid data length ManDiag session cmd and validate the response.            
            3. Post-condition: 
